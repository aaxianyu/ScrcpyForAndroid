...
- 升级: scrcpy 升级至 v4.1
- 新增: VP8/VP9 解码支持
- 重构: `AnnexBDecoder` 重命名为 `MediaCodecVideoDecoder`
- 重构: 拆分 `NativeCoreFacade`，新增 `VideoDecoderController`、`DecoderCapabilities`、`DecoderException`、`PersistentVideoRenderer`
- 修复: MTK 解码器在不支持的分辨率/旋转时崩溃（自动降级分辨率）
- 新增: `--ignore-video-encoder-constraints` 选项
- 新增: `scanFile()` 控制命令
- 修复: 应用打开时状态栏颜色
- 维护: 代码格式化
- 依赖: bcpkix-jdk18on v1.85, conscrypt-android v2.6.1

### 增强功能

- 新增: USB 有线 ADB 连接（USB 设备插拔监听、自动连接、隧道封装）
- 新增: 应用管理（安装/卸载/导出/停用/启用/清除数据/打开应用，所有应用弹窗 搜索/收藏/置顶排序）
...
- 新增: ADB 连接保活（前台服务通知保活 + 连接守护协程自动重连）
- 新增: 通知保活设置项（引导用户开启通知权限）
- 修复: Android 5/6 兼容性（ps 命令、printf→echo、ls 目录列表解析、动态初始路径）
- 修复: 文件管理器切换设备时不刷新（自动清除缓存并重新探测初始路径）
- 修复: 文件管理弹窗输入框光标跳转到开头
- 修复: 低延迟音频开关（及其他配置项）导致 Scrcpy 实例重建、无法进入全屏
- 修复: 文件管理器未连接设备时在 `/` 路径，连接后不自动跳转到内部存储
- 修复: 应用管理未连接设备时刷新按钮不实用且无法下拉刷新
- 修复: 应用管理刷新按钮显示 `%1$s`、错误消息未国际化
- 修复: 进程管理未连接设备时显示"未找到进程"而非"未连接设备"
- 修复: 深色模式下状态栏图标不可见
